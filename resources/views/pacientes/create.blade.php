    ...
    <form id="formPaciente" action="{{ route('pacientes.store') }}" method="POST" novalidate>
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-3">
                ...
                <div id="errorIdentidad" class="invalid-feedback" style="display:none;"></div>
                @error('identidad')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="mb-4">
            ...
            <textarea
                name="historial_quirurgico"
                id="historial_quirurgico"
                rows="2"
                maxlength="200"
...
document.getElementById('btnLimpiar').addEventListener('click', () => {
    ...
    // Limpiar mensaje de error específico identidad
    const errorIdentidad = document.getElementById('errorIdentidad');
    if(errorIdentidad) {
        errorIdentidad.textContent = '';
        errorIdentidad.style.display = 'none';
    }
});
</script>

{{-- Validación JS para año en identidad --}}
<script>
const form = document.getElementById('formPaciente');
...
function validarAnioIdentidad() {
    ...
    if (anio < anioMin || anio > anioMax) {
        errorIdentidad.textContent = `El año en la identidad debe estar entre ${anioMin} y ${anioMax}.`;
        errorIdentidad.style.display = 'block';
        inputIdentidad.setCustomValidity('Año inválido en identidad');
        return false;
    } else {
        errorIdentidad.textContent = '';
        errorIdentidad.style.display = 'none';
...
inputIdentidad.addEventListener('input', validarAnioIdentidad);

form.addEventListener('submit', function(event) {
    if (!validarAnioIdentidad()) {
        inputIdentidad.focus();
