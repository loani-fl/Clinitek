...
/* Contenedor Ultrasonidos en grid */
.examenes-grid {
    display: flex;
    flex-direction: column;
    gap: 0.15rem;
    ...
    <div class="row align-items-center">
        <div class="col-md-3 text-center">
            ...
            <div style="font-size: 1rem; font-weight: 700; color: #555;">
                Laboratorio Ultrasonidos Honduras
            </div>
        </div>
...
<div class="d-flex justify-content-center gap-3">
    ...
    <a href="{{ route('ultrasonidos.index') }}" class="btn btn-success px-4 py-2">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const fechaMax = new Date('2025-10-31');
    fechaInput.setAttribute('max', `${fechaMax.getFullYear()}-${String(fechaMax.getMonth()+1).padStart(2,'0')}-${String(fechaMax.getDate()).padStart(2,'0')}`);
    ...
    inputBuscar.addEventListener('input', ()=>{
        ...
        Array.from(lista.children).forEach(li=>{
            ...
            if(texto.includes(filtro)&&filtro!==''){li.style.display='block';visibleCount++;}else{li.style.display='none';}
        });
    ...
    checkboxes.forEach(chk=>chk.addEventListener('change',function(){
        const checkedCount=document.querySelectorAll('.examen-checkbox:checked').length;
        if(checkedCount>maxSeleccion){this.checked=false;showMensaje('No puede seleccionar más de 7 exámenes.','error');}
        actualizarTotal();
    }));

    function actualizarTotal(){let total=0; checkboxes.forEach(chk=>{if(chk.checked) total+=parseFloat(chk.dataset.precio)||0;}); document.getElementById('totalPrecio').textContent=total.toFixed(2);}
    actualizarTotal();
    ...
    form.addEventListener('submit', e=>{
        ...
        if(document.querySelectorAll('.examen-checkbox:checked').length===0){e.preventDefault(); showMensaje('Debe seleccionar al menos un examen para guardar.','error');}
    });

    document.getElementById('btnLimpiar').addEventListener('click', ()=>{
        inputBuscar.value=''; inputHiddenId.value=''; dpNombres.textContent=''; dpApellidos.textContent=''; dpIdentidad.textContent=''; dpGenero.textContent=''; document.getElementById('datosPaciente').style.display='none'; fechaInput.value=`${yyyy}-${mm}-${dd}`; checkboxes.forEach(chk=>chk.checked=false); actualizarTotal();
    });
});
</script>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    checkboxes.forEach(cb => {
        cb.addEventListener('change', () => {
            const checked = document.querySelectorAll('input[name="examenes[]"]:checked');
