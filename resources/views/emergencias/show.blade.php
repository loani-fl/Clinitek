...
@section('content')
    @php
        ...
        // Historial de emergencias previas (si hay paciente relacionado)
        $historial = $paciente
    ...
    <style>
        ...
        /* tabla historial */
        #historialTabla { display: none; margin-top: 1.25rem; }
    ...
    <div class="container mt-3">
        <div class="card custom-card shadow-sm border rounded-4">
            ...
            <div class="card-body px-3 py-3">
                ...
                <div class="text-center pt-3">
                    ...
                    <button type="button" class="btn btn-primary btn-sm px-4 shadow-sm" id="btnHistorial">Ver Historial</button>
                ...
                <div class="mt-4" id="historialTabla">
                    @if($historial->isEmpty())
                        <div class="alert alert-info text-center mt-2">No hay emergencias previas registradas para este paciente.</div>
                    @else
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered mt-2">
                                ...
                                @foreach($historial as $registro)
                                    <tr>
                                        <td>{{ $registro->nombre ?? ($paciente->nombre ?? '') }}</td>
                                        <td>{{ $registro->apellido ?? ($paciente->apellido ?? '') }}</td>
        ...
        @if($mostrarFoto)
            ...
            <script>
                // solo el script para cerrar modal si se hace click sobre la imagen (opcional)
        ...
        {{-- Script para mostrar/ocultar historial: siempre cargado --}}
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // inicialmente ocultamos visualmente la tabla (si está vacía se mostrará un mensaje)
                historialTabla.style.display = 'none';
                btnHistorial.textContent = 'Ver Historial';

                btnHistorial.addEventListener('click', () => {
                    const isHidden = historialTabla.style.display === 'none' || historialTabla.style.display === '';
                    if (isHidden) {
                        historialTabla.style.display = 'block';
                        btnHistorial.textContent = 'Ocultar Historial';
                        // opcional: baja la vista hacia la tabla
                        historialTabla.scrollIntoView({ behavior: 'smooth', block: 'start' });
                    } else {
                        historialTabla.style.display = 'none';
                        btnHistorial.textContent = 'Ver Historial';
                    }
