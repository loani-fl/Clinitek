...
{
    // Mostrar lista paginada de empleados con búsqueda simple
    public function index(Request $request)
    {
            ...
            ->when($buscar, function($query, $buscar) {
                $query->where(function($q) use ($buscar) {
                    $q->where('nombres', 'like', "%$buscar%")
                      ...
                      ->orWhereHas('puesto', function ($q2) use ($buscar) {
        ...
        return view('empleado.index', [$empleados]);  // cambiar 'empleados' por 'empleado' si tu carpeta es singular
    ...
    // Guardar nuevo empleado validando datos
    ...
    public function update(Request $request, string $id)
    {
        ...
        $rules = [
            ...
            'identidad' => [
                'required',
                'digits:13',
                // Unique excepto el registro actual
                "unique:empleados,identidad,$id",
                function ($attribute, $value, $fail) use ($departamentosValidos) {
                    $codigoDepartamento = substr($value, 0, 2);
                    if (!in_array($codigoDepartamento, $departamentosValidos)) {
                        $fail('El código del departamento en la identidad no es válido.');
