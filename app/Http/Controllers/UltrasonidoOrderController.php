...
public function guardarAnalisis(Request $request, $id)
{
    ...
    foreach ($examenesKeys as $index => $examenKey) {
        // Obtener el nombre del examen para mensajes amigables
    ...
    DB::beginTransaction();
    try {
        // Actualizar médico y estado en la orden
        ...
        foreach ($request->file('imagenes') as $examenIndex => $imagenesExamen) {
            // Usar el índice para obtener la CLAVE del examen
            ...
            foreach ($imagenesExamen as $index => $imagen) {
                // Guardar imagen en storage/app/public/ultrasonidos
